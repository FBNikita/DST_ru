...
function ApplyLocalizedFonts()
	...
	GLOBAL.HEADERFONT = FontNames.HEADERFONT
	...
	GLOBAL.CHATFONT = FontNames.CHATFONT
	GLOBAL.CHATFONT_OUTLINE = FontNames.CHATFONT_OUTLINE
	...
	GLOBAL.HEADERFONT = t.SelectedLanguage.."_hammerhead50"
	...
	GLOBAL.CHATFONT = t.SelectedLanguage.."_bellefair50"
	GLOBAL.CHATFONT_OUTLINE = t.SelectedLanguage.."_bellefair50_outline"
	if rawget(GLOBAL,"NEWFONT") then
...
function t.GetFromSpeechesHash(message, char)
	...
	if message and t.SpeechHashTbl[char] then
		...
		if char=="WATHGRITHR" then
			local tmp = message:gsub("[\246o]","o"):gsub("[\214O]","O") or message --подменяем и 1251 и UTF-8 версии
		...
		if umlautified then
			if rawget(GLOBAL, "GetSpecialCharacterPostProcess") then
				...
				message = message:gsub("о","o"):gsub("О","O") or message
			end
		end
	end
